$(function () {
    ...
    var $tel = $('#tel');
    var $role = $('#role');
    $username.val(passdata.username);
    $name.val(passdata.name);
    $tel.val(passdata.phoneNumber);
    $role.val(passdata.role);
    //隐藏提示
    $('#username, #password, #repassword, #name, #tel, #role').on('focus', function () {
    ...
    $('.btn_sure').click(function () {
        ...
        if (!name) {
            $name.parent().siblings('i').removeClass('hide').text('姓名不能为空');
            return false;
        } else if (name.length < 2) {
            $name.parent().siblings('i').removeClass('hide').text('姓名不能少于两个字');
            return false;
        }
        if (!tel) {
